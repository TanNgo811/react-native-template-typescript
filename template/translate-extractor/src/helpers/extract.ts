...
import {getMarkedPattern} from './get-marker-pattern';
...
export function extract(program: Command): void {
  ...
  input &&
    getFileList(input, include, exclude).forEach((file: string) => {
  ...
  languages.forEach((language: string) => {
    ...
    Object.entries(namespaces).forEach(([namespace, kebabizedNamespace]) => {
      const filePath: string = resolve(
        partials!,
        language,
        `${kebabizedNamespace}.json`,
      );
      if (existsSync(filePath)) {
        try {
          const existedKeys: Record<string, string> = JSON.parse(
      ...
      writeFileSync(
        filePath,
        JSON.stringify(updatedKeys, null, Number(indentSize)),
      );
      console.info(
        'Write %d keys to file %s',
